...
function obtenerCedulasPreregistradas() {
    global $conn;

    $cedulas = array();

    $query = "SELECT documento FROM persona WHERE pre_registro = 1"; // Filtra por pre_registro = 1
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["numero-documento"], $_POST["nombre-usuario"], $_POST["contrasena"], $_POST["rol"])) {
    ...
    if (empty($numeroDocumento) || empty($nombreUsuario) || empty($contrasena) || empty($rol)) {
        ...
        $sql_update = "UPDATE persona SET usuario=?, password_hash=?, IdRol=?, pre_registro=0 WHERE documento=?";
